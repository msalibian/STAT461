...
If we now repeat the calculations but after
transforming everything with `log`, which of course
makes everything symmetric and Gaussian again, we get:
```{r ex3.10}
(t0 <- mean(x))
```
Use bootstrap to estimate quantiles of the
...
Compare this with a CLT based CI:
```{r ex3.mlelog}
lo2 <- t0 - qnorm(.975)*sd(rho)
up2 <- t0 + qnorm(.975)*sd(rho)
c(lo=lo2, up=up2)
```
And, if we transform back to the `exp` scale:
```{r exp3.16}
rbind(c(lo=exp(lo1), up=exp(up1)),
      c(lo=exp(lo2), up=exp(up2)))
```
